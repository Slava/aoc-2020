...
type NState = M.Map (Int, Int, Int, Int) Int -- count of neighbors

fromString :: String -> State
fromString input = M.fromList [((x, y, 0, 0), c == '#') | (y, l) <- zip [0..] ls, (x, c) <- zip [0..] l]
...
transition st = M.fromList [(pos, True) | (pos, litLevel) <- M.toList litness, staysLit pos litLevel]
  where litness = foldr combine M.empty [litNeighbors pos | (pos, lit) <- M.toList st, lit]
        litNeighbors (x, y, z, w) = M.fromList [((x + dx, y + dy, z + dz, w + dw), 1) | dx <- [-1..1], dy <- [-1..1], dz <- [-1..1], dw <- [-1..1], dx /= 0 || dy /= 0 || dz /= 0 || dw /= 0]
        staysLit pos level
